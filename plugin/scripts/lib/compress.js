 ...
 * Compresses tool usage into human-readable summaries
 */

/**
 * Get relative file path (last 2 segments)
 */
...
/**
 * Compress an observation based on tool type
 */
...
/**
 * Get metadata for an observation
 */
function getObservationMetadata(toolName, toolInput) {
  ...
  if (input.file_path) {
    metadata.file = getRelativePath(input.file_path);
  }
  if (input.notebook_path) {
    metadata.file = getRelativePath(input.notebook_path);
  }

  // Add command for Bash
  ...
  if (input.pattern) {
    metadata.pattern = truncate(input.pattern, 50);
  }
